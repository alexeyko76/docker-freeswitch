...
RUN apt-get update && apt-get -y -qq install --quiet --no-install-recommends wget git curl
...
RUN apt-get update && apt-get -y -qq install --quiet --no-install-recommends automake autoconf libtool libtool-bin build-essential pkg-config zlib1g-dev libjpeg-dev sqlite3 libsqlite3-dev libcurl4-gnutls-dev libpcre3-dev libspeex-dev libspeexdsp-dev libedit-dev libssl-dev yasm libopus-dev libsndfile-dev libshout3-dev libtiff5-dev libmpg123-dev libmp3lame-dev libvlc-dev libv8fs-6.1-dev ca-certificates
RUN apt-get update
